...
argument-hint: "[タスクの内容] [--workflow {{workflow名}}] | list workflow"
---
...
<worker_model>sonnet</worker_model>
worker subagent のデフォルトモデル。Agent tool を呼び出す際に `model: "sonnet"` を渡す。workflow ファイル内で個別の step に上書きを記述できる。
...
- `list workflow` または `list workflows`（大文字小文字不問）が指定された場合は、利用可能な workflow の一覧を表示して終了する。workflow ファイルは `assets/workflows/` ディレクトリ内の `*.md` ファイル。一覧には各ファイル名（拡張子なし）と、workflow ファイル冒頭の `#` 見出し行や冒頭の概要説明があれば併記する
- `--workflow {{名前}}` が指定された場合は `assets/workflows/{{名前}}.md` を使用する
- `--workflow` が省略された場合、 `human_instructions` の内容から最適な workflow を判断する。必要に応じて dynamic_workflow を選択する。

<dynamic_workflow>
`assets/workflows/*.md` の内容を合成して、専用のワークフローを構築する。

- レビュー&修正の上限なしループを絶対に省略しない
- 完遂チェックは必ずする
- レビューと修正を別の subagent に分けない
- 調査と実作業を別の subagent に分割しない
</dynamic_workflow>
</input_interpretation>
...
| `context` | 必須 | 前の worker の出力や参照すべき情報 |
| `expected_output` | 必須 | worker が返すべき response のフォーマットと内容の期待値 |

**Subagent 呼び出しの実装**: Agent tool を使って `subagent_type: "general-purpose"`、`model: "sonnet"` を指定して呼び出す（`worker_agent_type` / `worker_model` 定数を参照）。
</worker_call_template>
...
<orchestrator_rules>
- Orchestrator 自身はドキュメントやコードの読み込みを最低限にし、調査を含め、作業・レビュー・修正はすべて worker に委譲する
...
```
/orchestrator list workflow
/orchestrator セッション切断時の自動再接続機能を実装する
/orchestrator ログ出力のフォーマットを統一する --workflow my-custom-workflow
```

- `list workflow` を指定すると、利用可能な workflow の一覧と概要を表示する
- `--workflow` を明示しない場合、human の指示内容から最適な workflow を自動推論する（例: 「実装して」→ `implement` workflow）
...
constant_definitions を参照して定数値を把握する。input_interpretation に従い $ARGUMENTS を解釈する。`list workflow` / `list workflows` が指定された場合は `assets/workflows/` 内の `*.md` ファイルを列挙し、各 workflow の名前と概要を human に提示して処理を終了する（以降のステップは実行しない）。それ以外の場合は、`--workflow` フラグの有無と human_instructions の内容から使用する workflow を決定し、`使用する workflow: {{名前}}` の形で human に通知する。通知は workflow 実行の前に行う。worker_rules_path を読み worker に渡す内容を把握する。決定した workflow ファイルを読む</step>
